function [tvec,Uvec] = RK2Problem2a(f,tspan,U0,k)
% [tvec,Uvec] = RK2Problem2a(f,tspan,U0,k)
% Another two-stage Runge-Kutta method
% Inputs
...
for n=1:N
   V1 = f(tvec(n), Uvec(:,n)); 
   V1 = V1(:);   % force into column vectors
   V2 = f(tvec(n)+2*k/3, Uvec(:,n) + 2*k*V1/3); 
   V2 = V2(:);
   Uvec(:,n+1) = Uvec(:,n)+k*V1/4 + 3*k*V2/4;
end
tvec = tvec';
